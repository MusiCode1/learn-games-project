...
try {
  if (typeof import.meta.url === "string" && import.meta.url) {
    currrntUrl = new URL(import.meta.url);
  }
} catch (e) {
  console.warn("Failed to parse import.meta.url:", e);
}
...
(() => {
  const cleanListener = addConfigListener((newConfig) => {
    ...
    if (isFullyKiosk() === false && isDemoAppList) {
      getExampleAppList()
        .then((data) => {
          exampleAppList = data;
          cleanListener();
        ...
        .catch((error) => {
          console.error(
            "Error fetching example app list during initialization:",
            error
          );
        });
    }
...
async function getFullyKioskPassword() {
  ...
  const password = window.fully?.getStringRawSetting("remoteAdminPassword");
