...
<div class="space-y-6" dir="rtl">
	...
	<!-- Setting: Hint Settings (Only if Word Display is Hidden) -->
	{#if settings.wordDisplayMode === 'hidden'}
		<div
			transition:slide={{ duration: 300, axis: 'y' }}
			class="border-t border-slate-100 pt-4 mt-4 mb-4"
		>
			<div class="flex items-center justify-between mb-4">
				<div class="space-y-1">
					<div class="font-bold text-slate-800">מנגנון רמז</div>
					<div class="text-sm text-slate-500">הצגת נורה לחשיפת המילה</div>
				</div>
				<button
					dir="ltr"
					aria-label="הפעלת מנגנון רמז"
					class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
					class:bg-blue-600={settings.hintEnabled}
					class:bg-slate-200={!settings.hintEnabled}
					onclick={() => (settings.hintEnabled = !settings.hintEnabled)}
				>
					<span
						class="absolute top-1 inline-block h-4 w-4 rounded-full bg-white transition-all duration-200 shadow-sm"
						class:left-1={!settings.hintEnabled}
						class:left-6={settings.hintEnabled}
					></span>
				</button>
			</div>
			...
			{#if settings.hintEnabled}
				<div
					transition:slide={{ duration: 300, axis: 'y' }}
					class="pr-2 space-y-4 border-r-2 border-slate-200 mr-2 bg-slate-50/50 p-2 rounded-l-lg"
				>
					<!-- Hint Duration -->
					<div class="flex items-center justify-between">
						<div class="space-y-1">
							<div class="font-medium text-slate-700">משך הרמז (שניות)</div>
							<div class="text-sm text-slate-500">כמה זמן המילה תהיה גלויה</div>
						</div>
						<input
							type="number"
							min="0.5"
							step="0.1"
							value={settings.hintDuration / 1000}
							onchange={(e) => (settings.hintDuration = parseFloat(e.currentTarget.value) * 1000)}
							class="w-20 px-3 py-1 bg-white rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700 font-medium text-center"
						/>
					</div>
					...
					<div class="flex items-center justify-between">
						<div class="space-y-1">
							<div class="font-medium text-slate-700">זמן צינון (שניות)</div>
							<div class="text-sm text-slate-500">זמן המתנה בין רמז לרמז (0 = ללא)</div>
						</div>
						<input
							type="number"
							min="0"
							step="1"
							bind:value={settings.hintCooldown}
							class="w-20 px-3 py-1 bg-white rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-700 font-medium text-center"
						/>
					</div>
				</div>
