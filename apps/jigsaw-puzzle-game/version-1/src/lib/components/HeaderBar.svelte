<!--
	בר עליון עם כפתור בית ומידע פאזל
-->
<script lang="ts">
...
>
  ...
  <div class="flex items-center gap-4 text-lg">
    {#if gameState.phase !== "INIT"}
      <span class="text-slate-400">חלקים:</span>
      <span class="font-bold text-green-400" dir="ltr">{gameState.connectedPieces}/{gameState.totalPieces}</span>

      <span class="text-slate-400">|</span>

      <span class="text-slate-400">פאזלים:</span>
      <span class="font-bold text-yellow-400">{gameState.puzzlesSolved}</span>
    {/if}
  </div>
