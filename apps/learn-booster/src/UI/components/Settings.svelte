<script lang="ts">
  ...
  async function persistConfig(closeSettings = true) {
    ...
    saveStatusTimeoutHandle = setTimeout(
      () => {
        if (saveStatus === "success" && closeSettings) {
          controller?.hide();
        }
        saveStatus = null;
      },
  ...
  async function applyProfileById(
    ...
    try {
      ...
      config = normalizeBoosterConfig(
        await configManager.updateConfig(structuredClone(profile.config))
      );
      newConfig = normalizeBoosterConfig(structuredClone(config));
  ...
  const createNewProfileFromCurrent = async () => {
    ...
    const name = window.prompt("תנו שם לפרופיל החדש:", suggestedName)?.trim();
    if (!name) return;
...
>
  <div class="p-4 md:p-6 space-y-4 md:space-y-6">
    ...
    <div id="settings-card" class="settings-card p-0 overflow-hidden">
      <div
        id="scroll-bar"
        class="bg-gray-50 p-4 md:p-6 space-y-4 md:space-y-6 max-h-[60vh] overflow-y-auto"
      >
          ...
          <section class="settings-card space-y-4 text-right">
            <div class="flex gap-2 flex-row items-center justify-between">
              ...
              >
                ...
                <input
                  type="checkbox"
                  class="!static h-[20px] w-[20px] rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            ...
            {#if isProfilesUiEnabled}
              <div class="space-y-3">
                <div class="flex flex-col space-y-1">
                  <label for="profileSelect" class="font-medium text-base">
                    בחירת פרופיל:
                  </label>
                  <select
                    id="profileSelect"
                  ...
                  {#if orderedProfiles.length === 0}
                    <p class="settings-help-text">
                      עדיין אין פרופילים שמורים. צרו פרופיל חדש כדי להתחיל.
                    </p>
                  {/if}
                </div>
                ...
                {#if hasUnsavedProfileChanges}
                  ...
                  >
                    בוצעו שינויים בפרופיל הפעיל שטרם נשמרו. לחצו על "שמור
                    לפרופיל הנוכחי" או על "שמור" בתחתית המסך.
                  </div>
                {/if}
            ...
            <section class="settings-card space-y-3 text-right">
              <div class="flex flex-col space-y-1 md:space-y-2">
                ...
                <input
                  ...
                  dir="ltr"
                  inputmode="url"
                  class="p-3 border rounded-lg text-right md:text-left bg-white text-base w-full touch-manipulation"
                />
              </div>
              <p class="settings-help-text">
                הזן כתובת אתר שהתלמידים יקבלו כמחזק לאחר הצלחה. ניתן להשאיר ריק
                כדי לא להציג מחזק.
              </p>
            </section>
          {/if}
            ...
            <section class="settings-card space-y-4 text-right">
              ...
              <div class="flex flex-col space-y-1 md:space-y-2">
                ...
                <p class="settings-help-text">
                  ...
                  כאן את שאם האפליקציה המלא (מכונה לפעמים 'חתימת אפליקציה')
                  להפעלה מדויקת של האפליקציה דרך Fully Kiosk. העתיקו אותו כפי
                  שמופיע באנדרואיד.
                </p>
              </div>
                ...
                >
                  ...
                  {:else}
                    <div class="flex-1 text-right">
                      ...
                      <p class="settings-help-text">
                        לא מצאנו את האפליקציה הזו ברשימת האפליקציות שמותקנות
                        במכשיר.
                      </p>
                    </div>
                    <div
