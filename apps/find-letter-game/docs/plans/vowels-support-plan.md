...
### פאזה 3 — הוספת 10 הניקודים הנותרים (data only, ללא TTS)

‏בעקבות פאזה 2: ‏ה-TTS לעיצור איכותי לא הצליח, ‏וההחלטה היא ‏לפצל את ייצור ה-MP3 לעבודה נפרדת לחלוטין. ‏לכן פאזה 3 ‏מצטמצמת ל-**הוספת ה-data בלבד** ‏(`Vowel[]` ‏+ ‏שמות ב-language.ts ‏+ ‏unicode handling) — ‏ה-`VowelSelectionGrid` הקיים יציג אוטומטית את כל הניקודים החדשים, ‏וכל הצירופים יפלו ל-Web Speech עד שייצור MP3 בעבודה הנפרדת.

**מה כן:**

‏- ‏10 entries חדשים ב-`vowels.ts`: ‏חיריק, סגול, חולם, שורוק, צירה, שווא, קמץ, קובוץ, חטף פתח, חטף סגול.
‏- ‏10 שמות בעברית ב-`language.ts`.
‏- ‏Unicode handling לשורוק (`וּ` = ‏ו + דגש). ‏ל-`Vowel.mark` ‏שלו זה מחרוזת של 2 chars (`'\u05D5\u05BC'`).
‏- ‏Tests.

**מה לא:**

‏- ‏אין ייצור MP3 בפאזה זו. ‏(תועלה לעבודה נפרדת.)
‏- ‏אין שינוי ב-`VowelSelectionGrid` (יציג אוטומטית).
‏- ‏אין שינוי ב-`makePair` ‏או ‏`generateDeck` (יעבדו אוטומטית).
‏- ‏אין סופיות (פאזה 4).
...
### עבודה נפרדת — ייצור MP3 לכל הניקודים

‏לא מסומנת כפאזה רשמית. ‏תיכלל בעבודה אחת מקיפה ‏כשהבעיה של "be vs b" תיפתר. ‏צפוי לכלול: ‏26 אותיות × ~11 ניקודים חדשים = ‏~280 קבצים (פחות אחרי שיתופי MP3).
