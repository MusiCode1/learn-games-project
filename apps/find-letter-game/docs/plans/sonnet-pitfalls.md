...
### 3.5 עיצור טהור ב-eleven_v3/Sarah — בעיה לא פתורה (2026-05-17)

‏eleven_v3 + קול Sarah **לא מסוגל להפיק עיצור עירום** (b, g, d בלי תנועה). כל speak text
שניסינו — שווא נח עברי (`'גְ'`), audio tags (`[Israeli accent] גְ`), תעתיק לטיני (`G`, `Gə`) —
הניב הברה עם תנועה (`ge`, `be`) ולא עיצור טהור.

‏ניסויים שנכשלו: שווא נח עברי, `[Israeli accent]`, `[grunt]`, `[breath]`, `[short ah]`, `ɛ` (IPA),
תעתיק לטיני קצר (`G`/`M`/`K`), תעתיק לטיני עם schwa (`Gə`/`Mə`).

‏ניסויים דרך proxy AAC + OneCLI ישיר — שניהם נתנו אותה תוצאה.

**‏הכלל:** ‏אל תבזבז זמן בניסיון להפיק עיצור טהור מ-eleven_v3/Sarah.
‏הצלילים הועלו כ-`be`-style (פונקציונלי, לא מושלם). ‏Follow-up ב-task נפרד.
