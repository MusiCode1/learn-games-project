...
### LetterSelectionGrid — הצגת אות בלי פתח

בגריד "אותיות להצגה" ב-`/settings`, כל אות הופיעה עם פתח (`אַ`, `בַּ`, `בַ`, `גַ`, …). זה היה הגיוני כשהמשחק תמך רק בפתח, אבל עכשיו שיש 10 ניקודים, ניקוד הפתח בתצוגה הזו מטעה — נראה כאילו הפתח הוא ניקוד "ראשי" של הבחירה.

**שינוי:** ב-`routes/_components/LetterSelectionGrid.svelte:55` — `{card.display}` (= `legacyDisplayPatah`, האות עם פתח) → `{card.letter.displayChar}` (האות בלבד, עם דגש/שיניים אם רלוונטי).

תוצאה: הגריד מציג עכשיו `א`, `בּ` (דגושה), `ב` (רפה), `ג`, `ד`, …, `שׁ` (ימנית), `שׂ` (שמאלית), `ת` — בלי ניקוד.

`LetterCard.svelte` (במשחק עצמו) **לא נגעו** — שם המשך להציג `card.display` עם הניקוד הנבחר. רק התצוגה של בחירת אותיות בהגדרות השתנתה.
...
המשתמש דיווח שהוא מבלבל; ההגדרה נשארה במסך `/settings` בלבד.
