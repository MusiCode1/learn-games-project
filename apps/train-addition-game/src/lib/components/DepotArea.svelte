...
<script lang="ts">
	...
	// האם הכפתור פעיל
	const isActive = $derived(gameState.state === 'BUILD_A' || gameState.state === 'ADD_B');

	// ספירה נוכחית והמטרה
	const current = $derived(gameState.state === 'BUILD_A' ? gameState.round.builtA : gameState.round.addedB);
	const target = $derived(gameState.state === 'BUILD_A' ? gameState.round.a : gameState.round.b);
...
<div class="flex flex-col items-center gap-4">
	...
	<button
		onclick={handleAdd}
		disabled={!isActive}
		class="group relative flex items-center justify-center gap-3 rounded-2xl px-10 py-5 text-xl font-bold text-white shadow-xl transition-all duration-300
			{isActive
			? `bg-gradient-to-br ${buttonColors} active:scale-95`
	...
	{#if isActive}
		<div class="flex items-center gap-2 text-lg font-medium text-slate-600">
			<span class="transition-all duration-300" class:scale-125={current > 0}>{current}</span>
			<span>/</span>
			<span>{target}</span>
			...
			<div class="ml-3 h-3 w-24 overflow-hidden rounded-full bg-slate-200">
				<div
					class="h-full rounded-full transition-all duration-500 ease-out {gameState.state === 'BUILD_A' ? 'bg-green-500' : 'bg-blue-500'}"
