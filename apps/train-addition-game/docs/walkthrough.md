...
### הגדרות Cooldown — זמן המתנה בטעות ואיפוס טיימר בלחיצה

הוספת שני פקדים חדשים בהגדרות המשחק לשליטה בהתנהגות ה-cooldown לאחר תשובה שגויה.

#### מה בוצע?

**1. חשיפת הגדרת זמן ההמתנה ב-UI**

- ההגדרה `cooldownMs` הייתה קיימת בקוד (ברירת מחדל 10 שניות) אך לא הייתה נגישה מפאנל ההגדרות.
- נוסף שדה מספרי "שניות המתנה בטעות" (1–30 שניות) בדף ההגדרות.

**2. הגדרה חדשה: איפוס טיימר בלחיצה (`resetCooldownOnTap`)**

- נוסף טוגל "איפוס טיימר בלחיצה" בהגדרות (כבוי כברירת מחדל).
- כשמופעל: לחיצה על כפתור תשובה בזמן cooldown מאפסת את הספירה מחדש (במקום להתעלם מהלחיצה), עם סאונד שגיאה כפידבק.
- כשכבוי: ההתנהגות הקיימת נשמרת — כפתורים חסומים לחלוטין בזמן cooldown.

**3. עדכון סכמת הגדרות**

- `TeacherSettings` הורחב עם `resetCooldownOnTap: boolean`.
- גרסת סכמה עלתה ל-6 עם מיגרציה תואמת לאחור.
